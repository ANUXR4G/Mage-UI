...
const SphereAnimation = () => {
  ...
  const fitElementToParent = (el: HTMLElement, padding?: number) => {
    ...
    function resize() {
      ...
      // Simple scale calculation without anime.js
      const pad = padding || 0;
  ...
  useEffect(() => {
    ...
    // Simple breathing animation using CSS animations
    spherePathEls.forEach((path, i) => {
      const pathElement = path as HTMLElement;
      pathElement.style.animation = `breathe 4s ease-in-out infinite`;
      pathElement.style.animationDelay = `${i * 0.1}s`;
    });

    // Simple intro animation
    spherePathEls.forEach((path, i) => {
      const pathElement = path as SVGPathElement;
      pathElement.style.strokeDasharray = pathElement.getTotalLength().toString();
      pathElement.style.strokeDashoffset = pathElement.getTotalLength().toString();
      pathElement.style.animation = `drawPath 3s ease-in-out forwards, breathe 4s ease-in-out infinite`;
      pathElement.style.animationDelay = `${i * 0.1}s, ${3 + i * 0.1}s`;
    });
...
export default function SphereAnimationPage() {
  return (

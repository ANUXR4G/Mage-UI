...
<ComponentPreview name="cursor-effects-bubble-cursor--docs" />
...
It will create a new file `bubble-cursor.tsx` inside the `components/mage-ui/cursor-effects` directory.

```bash 
mkdir -p components/mage-ui/cursor-effects && touch components/mage-ui/cursor-effects/bubble-cursor.tsx
```

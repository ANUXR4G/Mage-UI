...
<ComponentPreview name="card-lens-card--docs" />
...
It will create a new file `lens-card.tsx` inside the `components/mage-ui/card` directory.

```bash 
mkdir -p components/mage-ui/card && touch components/mage-ui/card/lens-card.tsx
```
...
```jsx file=<rootDir>/mage-ui/card/lens-card.tsx

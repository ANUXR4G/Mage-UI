...
<ComponentPreview name="card-shiftcard--docs" />
...
It will create a new file `shiftcard.tsx` inside the `components/mage-ui/card` directory.

```bash 
mkdir -p components/mage-ui/card && touch components/mage-ui/card/shiftcard.tsx
```

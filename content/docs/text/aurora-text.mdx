...
<ComponentPreview name="text-aurora-text--docs" />
...
It will create a new file `aurora-text.tsx` inside the `components/mage-ui/text` directory.

```bash 
mkdir -p components/mage-ui/text && touch components/mage-ui/text/aurora-text.tsx
```

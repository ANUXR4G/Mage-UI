...
It will create a new file `ai-button.tsx` inside the `components/mage-ui/button` directory.

```bash
mkdir -p components/mage-ui/button && touch components/mage-ui/button/ai-button.tsx
```
...
```jsx file=<rootDir>/mage-ui/button/ai-button.tsx

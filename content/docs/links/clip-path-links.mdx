...
description: Clip Path Links are stylized hyperlinks that use CSS clip-path to create unique shapes and dynamic hover effects.
---

<ComponentPreview name="links-clip-path-links--docs" />
...
It will create a new file `clip-path-links.tsx` inside the `components/mage-ui/links` directory.

```bash 
mkdir -p components/mage-ui/links && touch components/mage-ui/links/clip-path-links.tsx
```
